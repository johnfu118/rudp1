...
project.obj_dir ?= $(project.root_dir)objects
project.test_result_dir := $(project.root_dir)test_result
...
$(call dump_variable,project.src_dir)
...
# whether using distcc and ccache if available, default is true
builder.use_distcc ?= 1
builder.use_ccache ?= 1

# detect whether distcc is available
builder.distcc_detected = $(shell distcc >/dev/null 2>&1; error=$$?; if [ $$error != 127 ]; then echo y; fi)
...
ifneq ($(builder.use_distcc),)
ifneq ($(builder.distcc_detected),)
CPP_PREFIX += distcc
CC_PREFIX  += distcc
CXX_PREFIX += distcc
endif
...
get_target_build_type_flags = \
	$(if $(call is_debug,$(1)),\
		-g -fstack-protector,\
		$(if $($(1).optimize_flags),$($(1).optimize_flags),$(project.optimize_flags))\
	)
...
# Whenever compile flags are changed by external, eg, by command line or 
# environment variables, all impacted source files should be recompiled.
# using flags file to remember compile flags for each source file
...
$(call get_object,$(1),$(2)) : $(2) $(call get_flagfile,$(1),$(2)) $(call get_depend,$(1),$(2))
	...
	$$(call color_run,[$1] ,$(PREFIXED_CXX) -c $(call get_flags_of,$(1),$(2)) -o $$@ $$<,$(PREFIXED_CXX) $$<\t->  $$@)
endef
...
$(call get_object,$(1),$(2)) : $(2) $(call get_flagfile,$(1),$(2)) $(call get_depend,$(1),$(2))
	@$(call mkdir_if_not_exist,$(call get_source_objdir,$(1),$(2))); \
	$$(call color_run,[$1] ,$(PREFIXED_CC) -c $(call get_flags_of,$(1),$(2)) -o $$@ $$<,$(PREFIXED_CC) $$<\t->  $$@)
endef
...
codecheck/$(1)/$(2):
	@echo Checking $(2); \
	cppcheck -q --force --template 'gcc' --enable=style $(call get_cppcheck_includes,$(1),$(2)) $(2) 2>&1 |\
	while read line; do \
...
$(1).test:
	...
	$(call get_target_test_argv0,$(1)) $($(1).test_args) $(if $(call unify_bool_value,builder.test_output_xml),--gtest_output=xml:$(project.test_result_dir)/$(this_dir_part)/$(1).result.xml) --gtest_color=yes
endef
